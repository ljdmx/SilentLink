...
const Room: React.FC<RoomProps> = ({ config, onExit }) => {
      ...
      if (typeof e.data === 'string') {
        try {
          ...
          if (payload.type === 'chat' && encryptionKeyRef.current) {
            ...
            setMessages(prev => [...prev, { id: Date.now().toString(), senderId: remoteId, senderName: 'Peer', text, type: 'text', timestamp: Date.now() }]);
  ...
  const handleFileChunkReceive = async (data: ArrayBuffer) => {
    ...
    try {
      ...
      if (state.received >= state.size) {
        ...
        setMessages(prev => [...prev, { id: state.id, senderId: 'peer', senderName: '远端', blobUrl, type, fileName: state.name, timestamp: Date.now() }]);
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-background overflow-hidden relative font-sans selection:bg-primary/30">
                ...
                {role === 'none' ? (
                    <div className="text-center space-y-12 max-w-2xl animate-in zoom-in-95">
                        <div className="space-y-4">
                            <h3 className="text-3xl lg:text-5xl font-black text-white tracking-tighter uppercase italic">安全握手协议</h3>
                            <p className="text-gray-500 text-sm font-medium leading-relaxed max-w-lg mx-auto">物理隔离模式已激活。请选择一个节点入口。</p>
                        </div>
                    ...
                    <div className="w-full max-w-3xl animate-in slide-in-from-bottom-10">
                        <div className={`glass rounded-[3.5rem] p-10 lg:p-16 bg-black/40 border-t-8 transition-all ${role === 'initiator' ? 'border-t-primary' : 'border-t-accent'}`}>
                            {role === 'initiator' ? (
                                <div className="space-y-10">
                                    {handshakeStep === 2 && (
                                        <div className="space-y-10 animate-in zoom-in-95">
                                            ...
                                            <div className="space-y-4">
                                                <h4 className="text-[11px] font-black text-white uppercase tracking-widest px-2">回填 Answer 代码</h4>
                                                <div className="relative">
                                                    <textarea placeholder="粘贴回执代码..." value={remoteSDPInput} onChange={(e) => setRemoteSDPInput(e.target.value)} className="w-full h-32 bg-black/60 border border-white/10 rounded-[2rem] p-6 text-[10px] font-mono text-accent outline-none resize-none transition-all" />
                                                    <button onClick={finalizeAsInitiator} disabled={!remoteSDPInput} className="absolute bottom-4 right-4 h-12 px-8 bg-accent text-black rounded-xl font-black uppercase tracking-widest active:scale-95 disabled:opacity-30">连接</button>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-10">
                                    <textarea placeholder="粘贴 Offer 代码..." value={remoteSDPInput} onChange={(e) => setRemoteSDPInput(e.target.value)} className="w-full h-32 bg-black/60 border border-white/10 rounded-[2rem] p-6 text-[10px] font-mono text-accent outline-none" />
                                    <button onClick={handleOfferAndReply} className="w-full h-16 bg-white/5 border border-white/10 text-white rounded-[2rem] font-black uppercase tracking-widest">解析 Offer</button>
                                    {handshakeStep === 2 && (
                                        <button onClick={() => {navigator.clipboard.writeText(localSDP); setIsCopied(true); setTimeout(()=>setIsCopied(false), 2000);}} className="w-full h-16 bg-accent text-black rounded-[2rem] font-black uppercase tracking-widest">{isCopied ? '已复制 Answer' : '复制回执代码'}</button>
                                    )}
                                </div>
                            )}
                        </div>
              ...
              <div className="absolute inset-0 z-0">
                {remoteParticipant ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center space-y-4 opacity-50">
                    <span className="material-symbols-outlined text-6xl animate-pulse">hourglass_top</span>
                    <p className="text-xs font-black uppercase tracking-[0.5em]">等待对方流接入...</p>
                  </div>
                )}
              </div>

              {/* 悬浮小窗：本地画面 */}
              <div className="absolute bottom-6 left-6 w-48 lg:w-64 aspect-video z-50 transition-all duration-500 hover:scale-110">
                {localParticipant && (
        ...
        <div className={`fixed inset-0 lg:static lg:inset-auto lg:w-96 glass transform transition-all duration-300 z-[110] flex flex-col lg:m-2 lg:rounded-2xl overflow-hidden ${isChatOpen ? 'translate-y-0 opacity-100 shadow-3xl' : 'translate-y-full lg:hidden opacity-0 scale-95 pointer-events-none'}`}>
             <div className="h-14 flex items-center justify-between px-6 border-b border-white/5 bg-black/40 shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400">E2EE 多媒体隧道</h3>
                </div>
      ...
      {connectionStatus === 'connected' && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 p-3 glass rounded-[2rem] z-[100] shadow-2xl shadow-black">
    ...
    <div className="flex flex-col h-full bg-[#0a0f14]">
        ...
        {messages.map(m => (
          <div key={m.id} className={`flex flex-col ${m.senderId === 'local' ? 'items-end' : 'items-start'}`}>
             ...
             <div className={`rounded-2xl max-w-[90%] overflow-hidden shadow-xl ${m.senderId === 'local' ? 'bg-primary text-white rounded-tr-none' : 'bg-white/5 border border-white/10 text-gray-300 rounded-tl-none'}`}>
               {m.type === 'text' && <p className="px-4 py-3 text-xs leading-relaxed">{m.text}</p>}
               {m.type === 'image' && <img src={m.blobUrl} className="w-full h-auto cursor-pointer hover:opacity-90" onClick={() => window.open(m.blobUrl)} />}
               ...
               {(m.type === 'image' || m.type === 'video' || m.type === 'file') && (
                 <div className="px-4 py-2 bg-black/40 flex items-center justify-between gap-4 border-t border-white/5">
                   <p className="text-[9px] font-mono truncate opacity-60 uppercase">{m.fileName}</p>
                   <a href={m.blobUrl} download={m.fileName} className="material-symbols-outlined text-sm hover:text-accent">download</a>
                 </div>
               )}
             </div>
          </div>
      ...
      <div className="p-4 bg-black/60 border-t border-white/5 shrink-0">
        <form onSubmit={(e) => { e.preventDefault(); if (text.trim()) { onSend(text); setText(''); } }} className="relative flex gap-2">
          ...
          <input className="flex-1 h-12 bg-white/5 border border-white/10 rounded-xl px-4 text-xs text-white focus:outline-none focus:border-primary/50 transition-all" placeholder="输入消息..." value={text} onChange={(e) => setText(e.target.value)} />
          <button type="submit" disabled={!text.trim()} className="shrink-0 size-12 bg-primary text-white rounded-xl flex items-center justify-center disabled:opacity-30 active:scale-95 transition-all"><span className="material-symbols-outlined">send</span></button>
        </form>
      </div>
    </div>
  );
